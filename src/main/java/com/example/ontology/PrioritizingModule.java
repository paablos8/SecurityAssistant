		...
		for (int i = 0; i < recommendations.size(); i++) {
			...
			for (int j = 0; j < threats.size(); j++) {
				...
				while (iter.hasNext()) {
					...
					if (typeOfThreat
							.equals("http://securityontology.sba-research.org/securityontology.owl#TopLevelThreat")) {
						priorityScore = priorityScore + 20;
					}
					// Add + 10, if the threat is of the type "LowLevelThreat".
					else if (typeOfThreat
							.equals("http://securityontology.sba-research.org/securityontology.owl#LowLevelThreat")) {
				...
				if (currentThreatIndividual.hasProperty(givesRiseToThreat)) {
					...
					while (iter_2.hasNext()) {
						...
						Individual threatCanBeConsequenceIndividual = base
								.getIndividual(threatCanBeConsequence.getURI());
						...
						while (iter_3.hasNext()) {
							...
							if (typeOfThreat_2.equals(
									"http://securityontology.sba-research.org/securityontology.owl#TopLevelThreat")) {
								priorityScore = priorityScore + 5;
								System.out.println("The threat also gives rise to the threat: "
										+ threatCanBeConsequence.getLocalName() + " which is a "
										+ threatType_2.getLocalName());
								System.out.println(
										"This gives additional 5 Priority Points - current Score is: " + priorityScore);
							}

							else if (typeOfThreat_2.equals(
									"http://securityontology.sba-research.org/securityontology.owl#LowLevelThreat")) {
								priorityScore = priorityScore + 3;
								System.out.println("The threat also gives rise to the threat: "
										+ threatCanBeConsequence.getLocalName() + " which is a "
										+ threatType_2.getLocalName());
								System.out.println(
										"This gives additional 3 Priority Points - current Score is: " + priorityScore);
				...
				System.out.println("The recommendation: " + currentRecommendation.getTitle() + " has the threat: "
						+ currentThreatIndividual.getLocalName());
